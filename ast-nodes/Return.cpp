...
string Return::buildIR(CFG * cfg){
	...
	if(returnType->getTtext() == "void"){
		if(expr != nullptr){
			cerr << "Erreur dans le retour void" << endl;
		}
	}else{
		if(expr != nullptr && returnType->getTtext() == expr->getType->getText()){
			...
			cfg->add_IRInstr(IRInstr::ldconst,expr->getType(),params1);
			vector<string>params2={left,"!bp",left};
			cfg->add_IRInstr(IRInstr::add,expr->getType(),params2);
			vector<string> params3={left,value};
			cfg->add_IRInstr(IRInstr::wmem,expr->getType(),params3);
		}else{
